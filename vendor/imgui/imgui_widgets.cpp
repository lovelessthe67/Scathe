...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImVec2 pos = window->DC.CursorPos + ImVec2(5.0f, 0.0f); // :3
    ...
    if (is_visible)
    {
        ...
        window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, IM_COL32(0, 0, 0, 255), 0.0f);
        window->DrawList->AddRectFilled(check_bb.Min + ImVec2(1, 1), check_bb.Max - ImVec2(1, 1), *v ? IM_COL32(100, 160, 200, 255) : IM_COL32(51, 51, 51, 255), 0.0f);

        if (*v)
            window->DrawList->AddRectFilled(check_bb.Min + ImVec2(2, 2), check_bb.Max - ImVec2(2, 2), IM_COL32(137, 207, 240, 255), 0.0f);
        else
            window->DrawList->AddRectFilled(check_bb.Min + ImVec2(2, 2), check_bb.Max - ImVec2(2, 2), IM_COL32(12, 12, 12, 255), 0.0f); // 10,10,10, was so dark
    } 

    const ImVec2 label_pos = ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePadding.y - 3);
    if (g.LogEnabled)
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const float w = GetContentRegionAvail().x - 8;

    const float lh = label_size.y + style.ItemSpacing.y;
    const ImRect bb(window->DC.CursorPos + ImVec2(5, lh), window->DC.CursorPos + ImVec2(w, lh + 14.0f));
    const ImRect total_bb(window->DC.CursorPos + ImVec2(5, 0), bb.Max + ImVec2(0, 1));

    ItemSize(total_bb, 4.0f); 
    /*
                ,.  ,.
            ||  ||
           ,''--''.
   ...
   */
    ...
    if (label_size.x > 0)
    {
        ImVec2 lp = ImVec2(bb.Min.x, bb.Min.y - lh);
        ImVec2 ls = label_size;
        window->DrawList->AddText(lp + ImVec2(-1, 0), IM_COL32(0, 0, 0, 255), label);
        window->DrawList->AddText(lp + ImVec2(1, 0), IM_COL32(0, 0, 0, 255), label);
        window->DrawList->AddText(lp + ImVec2(0, -1), IM_COL32(0, 0, 0, 255), label);
        window->DrawList->AddText(lp + ImVec2(0, 1), IM_COL32(0, 0, 0, 255), label);
        RenderText(lp, label);
    }

    const ImU32 tc = IM_COL32(12, 12, 12, 255);
    const ImU32 bc = IM_COL32(6, 7, 7, 255);
    const float vx2 = ImMax(bb.Min.x, bb.Max.x - arrow_size);

    RenderNavCursor(bb, id);

    if (!(flags & ImGuiComboFlags_NoPreview))
    {
        window->DrawList->AddRectFilledMultiColor(bb.Min, ImVec2(vx2, bb.Max.y), tc, tc, bc, bc);
    }
    ...
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ...
        window->DrawList->AddRectFilledMultiColor(ImVec2(vx2, bb.Min.y), bb.Max, tc, tc, bc, bc);


        if (vx2 + arrow_size - style.FramePadding.x <= bb.Max.x)
        {
            float cx = vx2 + arrow_size * 0.5f + 1.0f;
            float cy = bb.Min.y + (bb.Max.y - bb.Min.y) * 0.5f;

            float hw = 4.0f;
            float hh = 2.0f;

            // this will be so gay but it is what it is man

            ImVec2 p1 = ImVec2(cx - hw, cy - hh);
            ImVec2 p2 = ImVec2(cx + hw, cy - hh);
            ImVec2 p3 = ImVec2(cx, cy + hh);

            if (!popup_open)
            {
                p1 = ImVec2(cx - hw, cy - hh);
                p2 = ImVec2(cx + hw, cy - hh);
                p3 = ImVec2(cx, cy + hh);
            }
            else
            {
                p1 = ImVec2(cx - hw + 1, cy + hh);
                p2 = ImVec2(cx + hw - 1, cy + hh);
                p3 = ImVec2(cx, cy - hh + 1);
            }
            window->DrawList->AddTriangleFilled(p1 + ImVec2(-1, 0), p2 + ImVec2(-1, 0), p3 + ImVec2(-1, 0), IM_COL32(0, 0, 0, 255));
            window->DrawList->AddTriangleFilled(p1 + ImVec2(1, 0), p2 + ImVec2(1, 0), p3 + ImVec2(1, 0), IM_COL32(0, 0, 0, 255));
            window->DrawList->AddTriangleFilled(p1 + ImVec2(0, -1), p2 + ImVec2(0, -1), p3 + ImVec2(0, -1), IM_COL32(0, 0, 0, 255));
            window->DrawList->AddTriangleFilled(p1 + ImVec2(0, 1), p2 + ImVec2(0, 1), p3 + ImVec2(0, 1), IM_COL32(0, 0, 0, 255));
            window->DrawList->AddTriangleFilled(p1, p2, p3, text_col);
        }
    }

    window->DrawList->AddLine(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Min.y), IM_COL32(55, 55, 55, 255), 1.0f);
    if (!popup_open)
    {
        float sep_x = vx2;

        window->DrawList->AddLine(ImVec2(bb.Min.x, bb.Max.y), ImVec2(bb.Max.x + 1, bb.Max.y), IM_COL32(55, 55, 55, 255), 1.0f);
        window->DrawList->AddLine(ImVec2(sep_x, bb.Min.y), ImVec2(sep_x, bb.Max.y), IM_COL32(55, 55, 55, 255), 1.0f);

    }
    else
        window->DrawList->AddLine(ImVec2(vx2, bb.Max.y), ImVec2(bb.Max.x, bb.Max.y), IM_COL32(55, 55, 55, 255), 1.0f);
    window->DrawList->AddLine(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Min.x, bb.Max.y), IM_COL32(55, 55, 55, 255), 1.0f);
    window->DrawList->AddLine(ImVec2(bb.Max.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), IM_COL32(55, 55, 55, 255), 1.0f);

    // Custom preview
    if (flags & ImGuiComboFlags_CustomPreview)
    {
        g.ComboPreviewData.PreviewRect = ImRect(bb.Min.x, bb.Min.y, vx2, bb.Max.y);
    ...
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
    {
        ...
        ImVec2 text_size = CalcTextSize(preview_value, NULL, true);
        float text_y = bb.Min.y + (14.0f - text_size.y) * 0.5f;
        RenderTextClipped(ImVec2(bb.Min.x + style.FramePadding.x, text_y), ImVec2(vx2, bb.Max.y), preview_value, NULL, NULL);
    }
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    if (g.NextWindowData.HasFlags & ImGuiNextWindowDataFlags_HasSizeConstraint)
    {
        ...
        g.NextWindowData.SizeConstraintRect.Max.x = w;
    }
    else
    {
        ...
        ImVec2 constraint_min(w, 0.0f), constraint_max(w, FLT_MAX);
    ...
    ImRect bb_offset = bb;
    bb_offset.Min.y -= 2.0f;
    bb_offset.Max.y -= 2.0f;
    ...
    if (ImGuiWindow* popup_window = FindWindowByName(name))
        if (popup_window->WasActive)
        {
            ...
            ImVec2 pos = FindBestWindowPosForPopupEx(bb_offset.GetBL(), size_expected, &popup_window->AutoPosLastDirection, r_outer, bb_offset, ImGuiPopupPositionPolicy_ComboBox);
            SetNextWindowPos(pos);
        }
    // We don't use BeginPopupEx() solely because we have a custom name string, which we could make an argument to BeginPopupEx()
    ImGuiWindowFlags window_flags = ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_Popup | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoMove;
    PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(g.Style.FramePadding.x, 5.0f));
    bool ret = Begin(name, NULL, window_flags);

    if (ret)
    {
        ImGuiWindow* window = GetCurrentWindow();
        ImVec2 pmin = window->Pos;
        ImVec2 pmax = ImVec2(window->Pos.x + window->Size.x, window->Pos.y + window->Size.y);

        window->DrawList->AddRectFilledMultiColor(
            pmin,
            pmax,
            IM_COL32(7, 7, 7, 255),       // ends 777
            IM_COL32(7, 7, 7, 255),       // ends 7
            IM_COL32(15, 15, 15, 255),    // ends 1217212
            IM_COL32(15, 15, 15, 255)     // ends 1721212
        );

        window->DrawList->AddLine(ImVec2(pmin.x, pmin.y), ImVec2(pmin.x, pmax.y), IM_COL32(55, 55, 55, 255), 1.0f); 
        window->DrawList->AddLine(ImVec2(pmax.x - 1, pmin.y), ImVec2(pmax.x - 1, pmax.y), IM_COL32(55, 55, 55, 255), 1.0f);
        window->DrawList->AddLine(ImVec2(pmin.x, pmax.y - 1), ImVec2(pmax.x, pmax.y - 1), IM_COL32(55, 55, 55, 255), 1.0f);

        // just realized now, coulda have just made a rect and draw it before the rectfilledmulticolor and make its p1 Y 1 pixel down so that mutlicol will be on top of it
        // lol, its fine this way also works :p
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const float x_padding = 8.0f;
    const float y_padding = 4.0f;
    const ImVec2 pos = window->DC.CursorPos;

    const float content_width = GetContentRegionAvail().x; // so you might be asking
    // why not window->WindowPadding.x. because we hardcoded it on beginchild: WorkRect.Max.x -= 5.0f
    // so we are using getcontentregionavail which works pretty fine

    const ImRect frame_bb(pos + ImVec2(x_padding - 2.0f, label_size.y + y_padding), pos + ImVec2(content_width - x_padding - 2.0f, label_size.y + y_padding + slider_height));
    const ImRect total_bb(pos, pos + ImVec2(content_width, label_size.y + y_padding + slider_height + 1.0f));
    ...
    window->DrawList->AddRect(frame_bb.Min - ImVec2(1, 1), frame_bb.Max + ImVec2(1, 1), IM_COL32(55, 55, 55, 255), 0.0f);
    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, IM_COL32(12, 12, 12, 255), 0.0f);
    ...
    float grab_width = grab_bb.Max.x - frame_bb.Min.x;
    if (grab_bb.Max.x >= frame_bb.Max.x - 0.1f)
        grab_width = frame_bb.Max.x - frame_bb.Min.x;

    if (grab_width > 0.0f)
        window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(frame_bb.Min.x + grab_width, frame_bb.Max.y), IM_COL32(137, 207, 240, 255), 0.0f);

    if (label_size.x > 0.0f)
    {
        ImVec2 label_pos = ImVec2(frame_bb.Min.x - 1.0f, pos.y);
        for (int x = -1; x <= 1; x++)
            for (int y = -1; y <= 1; y++)
                if (x != 0 || y != 0)
                    window->DrawList->AddText(ImVec2(label_pos.x + x, label_pos.y + y), IM_COL32(0, 0, 0, 255), label);
    ...
    ImVec2 value_pos = ImVec2(frame_bb.Max.x - value_size.x + 1.0f, pos.y);
    for (int x = -1; x <= 1; x++)
        for (int y = -1; y <= 1; y++)
            if (x != 0 || y != 0)
                window->DrawList->AddText(ImVec2(value_pos.x + x, value_pos.y + y), IM_COL32(0, 0, 0, 255), value_buf, value_buf_end);
    window->DrawList->AddText(value_pos, GetColorU32(ImGuiCol_Text), value_buf, value_buf_end);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        float rx = GetContentRegionAvail().x - 8;
        ImVec2 p = ImVec2(window->DC.CursorPos.x - 13 + rx, window->DC.CursorPos.y + 1);
        ImRect bb(p, p + ImVec2(12, 12));

        bool hovered, held;
        bool pressed = ButtonBehavior(bb, window->GetID("##ColorButton"), &hovered, &held);

        int r = (int)(col_v4.x * 255) - 50;
        int gv = (int)(col_v4.y * 255) - 50;
        int bv = (int)(col_v4.z * 255) - 50;
        if (r < 0) r = 0;
        if (gv < 0) gv = 0;
        if (bv < 0) bv = 0;
        ImU32 dc = IM_COL32(r, gv, bv, 255);


        window->DrawList->AddRectFilled(p + ImVec2(1, 1), p + ImVec2(11, 11), ColorConvertFloat4ToU32(col_v4));
        window->DrawList->AddRect(p + ImVec2(1, 1), p + ImVec2(11, 11), IM_COL32(0, 0, 0, 60)); // ig
        window->DrawList->AddRect(p, p + ImVec2(12, 12), IM_COL32(0, 0, 0, 255), 0.0f, 0, 1.0f);



        window->DrawList->AddRectFilled(p + ImVec2(1, 1), p + ImVec2(6, 6), dc);
        window->DrawList->AddRectFilled(p + ImVec2(6, 6), p + ImVec2(11, 11), dc);

        ItemSize(bb);
        ItemAdd(bb, window->GetID("##ColorButton"));
        ...
        if (pressed)
        {
            if (!(flags & ImGuiColorEditFlags_NoPicker))
            {
                ...
                OpenPopup("picker");
                SetNextWindowPos(bb.GetBL() + ImVec2(0.0f, style.ItemSpacing.y));
            }
        }
        if (!(flags & ImGuiColorEditFlags_NoOptions))
        ...
        if (BeginPopup("picker"))
        {
            if (g.CurrentWindow->BeginCount == 1)
            {
                ...
                ImVec2 popup_min = popup_window->Pos;
                ImVec2 popup_max = popup_window->Pos + popup_window->Size;

                popup_window->DrawList->AddRectFilled(popup_min, popup_max, IM_COL32(15, 15, 15, 255));
                popup_window->DrawList->AddRect(popup_min, popup_max, IM_COL32(55, 55, 55, 255), 0.0f, 0, 1.0f);
                popup_window->DrawList->AddRect(popup_min + ImVec2(1, 1), popup_max - ImVec2(1, 1), IM_COL32(0, 0, 0, 255), 0.0f, 0, 1.0f);
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    const ImU32 hues[7] = { IM_COL32(255,0,0,255), IM_COL32(255,255,0,255), IM_COL32(0,255,0,255), IM_COL32(0,255,255,255), IM_COL32(0,0,255,255), IM_COL32(255,0,255,255), IM_COL32(255,0,0,255) };

    ImVec4 hue_color_f(1, 1, 1, 1);
    ColorConvertHSVtoRGB(H, 1, 1, hue_color_f.x, hue_color_f.y, hue_color_f.z);
    ImU32 hue_color32 = ColorConvertFloat4ToU32(hue_color_f);
    ImU32 user_col32_striped_of_alpha = ColorConvertFloat4ToU32(ImVec4(R, G, B, 1));

    ImVec2 sv_cursor_pos;

    draw_list->AddRectFilledMultiColor(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), IM_COL32(255, 255, 255, 255), hue_color32, hue_color32, IM_COL32(255, 255, 255, 255));
    draw_list->AddRectFilledMultiColor(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), 0, 0, IM_COL32(0, 0, 0, 255), IM_COL32(0, 0, 0, 255));

    draw_list->AddRect(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), IM_COL32(30, 30, 30, 255), 0.0f, 0, 1.0f);

    ImVec4 hover_color = ColorConvertU32ToFloat4(IM_COL32(137, 207, 240, 255));
    ...
    for (int i = 0; i < 6; ++i)
        draw_list->AddRectFilledMultiColor(ImVec2(bar0_pos_x, picker_pos.y + i * (sv_picker_size / 6)), ImVec2(bar0_pos_x + bars_width, picker_pos.y + (i + 1) * (sv_picker_size / 6)), hues[i], hues[i], hues[i + 1], hues[i + 1]);
    ...
    ImVec4 hue_hover_col = ColorConvertU32ToFloat4(IM_COL32(137, 207, 240, 255));
    ...
    ImVec4 line_col = ColorConvertU32ToFloat4(IM_COL32(137, 207, 240, 255));
    line_col.w = 1.0f;
    ImU32 line_col32 = ColorConvertFloat4ToU32(line_col);
    draw_list->AddLine(ImVec2(bar0_pos_x, anim_hue_y), ImVec2(bar0_pos_x + bars_width, anim_hue_y), IM_COL32(0, 0, 0, 255), 3.0f);
    ...
    draw_list->AddCircle(sv_cursor_pos, sv_cursor_rad + 1, IM_COL32(128, 128, 128, 255), sv_cursor_segments);
    draw_list->AddCircle(sv_cursor_pos, sv_cursor_rad, IM_COL32(255, 255, 255, 255), sv_cursor_segments);
    ...
    if (alpha_bar)
    {
        ...
        ImVec4 alpha_hover_col = ColorConvertU32ToFloat4(IM_COL32(137, 207, 240, 255));
        ...
        draw_list->AddLine(ImVec2(anim_alpha_x, bar1_bb.Min.y), ImVec2(anim_alpha_x, bar1_bb.Max.y), IM_COL32(0, 0, 0, 255), 3.0f);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (is_visible)
    {
        ...
        if (!selected)
        {
            ImVec4 col = ColorConvertU32ToFloat4(text_col);
            col.w *= 0.5f;
            text_col = ColorConvertFloat4ToU32(col);
        }
