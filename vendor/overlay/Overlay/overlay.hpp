...
typedef struct screen {
  ...
  bool operator==(const screen &screen) {
    return x == screen.x && y == screen.y && width == screen.width &&
           height == screen.height;
  }

  bool operator!=(const screen &screen) { return !(*this == screen); }
} screen_t;
class overlay {
public:
  static bool render();
...
private:
  ...
  static LRESULT WINAPI window_proc(HWND hWnd, UINT msg, WPARAM wParam,
                                    LPARAM lParam);
